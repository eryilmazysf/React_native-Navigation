...
const Main = (props) => {
  const renderPostData = ({item}) => {
    return (
      <PostCard
        post={item}
        onSelect={
          () => props.navigation.navigate('PostPage', {selectedPost: item}) //postPage git ve item da gotur
        }
      />
    );
  };
  ...
  return (
    <SafeAreaView>
      {/* <Text style={{fontSize: 30, color: 'purple'}}>Main</Text>
      <Button
        title="GO!!"
      ...
      <View>
        <FlatList
          ...
          // border eklemek icin among of images
